data{
  ...
  int<lower=1> n_dbands;      // Number of censored detection intervals
  int<lower=0> y[n_sites,n_dbands];  // Counts by survey and observation interval
  
// Covariates
// int<lower=0> n_bd;        // Number of detection fixed effects
// matrix[n_sites,n_bd] Xd;  // Explanatory variables for detection
  // Data
  real<lower=0> tau[n_dbands];       // Endpoints of observation intervals
}

transformed data {
int<lower=0> ii[n_sites];         // An index identifying where in the vectorized data each survey begins (minus 1)
int<lower=0> yv[n_sites*n_dbands];  // Vectorized data
...
transformed parameters {
  ...
  vector<upper=0>[n_dbands] log_p[n_sites]; // Interval-specific log(detection probability)
  vector[n_sites*n_dbands] log_mu_ab;       // Interval-specific log(Expected count = lambda * Pr(detection in interval))
...
generated quantities{
  // vector[n_sites*n_dbands] log_lik;
  // real phi;
  matrix[n_sites,n_dbands] phi_est_i;
  vector[n_sites] phi;
  for(s in 1:n_sites){
